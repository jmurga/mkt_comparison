	...
	for i in range(0,len(samples),1):
		...
		if(dfC.shape[0] == 2):
			if(i == 0):
				...
				tmp = tmp + ref.get_spliced_seq(list(ref.keys())[0], dfC[dfC['strand']=='-']['coordinates'][1]).reverse.complement.seq
				f.write('>' + list(ref.keys())[0] + '\n' + tmp +'\n')
				# multiFasta
				tmp = file.get_spliced_seq(samples[i], dfC[dfC['strand']=='+']['coordinates'].iloc[0]).seq
				tmp = tmp + file.get_spliced_seq(samples[i], dfC[dfC['strand']=='-']['coordinates'][1]).reverse.complement.seq
				f.write('>' + samples[i] + '\n' + tmp +'\n')
				...
				tmp = tmp + out.get_spliced_seq(list(out.keys())[0], dfC[dfC['strand']=='-']['coordinates'].iloc[0]).reverse.complement.seq
