...
if __name__ == "__main__":
	...
	with NamedTemporaryFile("w") as slim_file:
		...
		for i in range(30,args.replica,1):
			print(i)
			...
			if(args.recipe=='baseline'):
				...
				# divergenceAndTrueAlpha.append(div)
				# listDaf.append(daf)
			elif(args.recipe=='wdFractionTwoElements'):

				slimResults = subprocess.run(["/home/jmurga/mkt/201902/software/SLiM3.3/slim", "-s", str(random.randint(1, 10**13)),slim_file.name],universal_newlines=True,stdout=subprocess.PIPE)

				rawResults = slimResults.stdout.split('\n')
				rawResults = rawResults[18:]
				...
				# True alpha values
				rawTrueAlpha = float(rawResults[2:3][0].split(':')[1])
				standardAlpha = float(rawResults[3:4][0].split(':')[1])
				
				# Daf values
				rawDaf = rawResults[4:-4]
				m0 = float(rawResults[-4:-1][0].split(':')[1])
				mi = float(rawResults[-4:-1][1].split(':')[1])
				rawB = float(rawResults[-4:-1][2].split(':')[1])
				...
				div = pd.DataFrame({'Di':rawD,'D0':rawD0,'trueAlpha':rawTrueAlpha,'mi':mi,'m0':m0,'b':rawB},index=[0],dtype=float)

				standardMkt = np.array([[daf[['P0']].sum().values[0],rawD0],[daf[['Pi']].sum().values[0],rawD]])

				synonymousRatio = standardMkt[0,0]/m0    
				nonSynonymousRatio = standardMkt[1,0]/mi  

				f = (nonSynonymousRatio/synonymousRatio)
				b = (daf.Pwd.sum()/daf.P0.sum())*(m0/mi)  
				fN = f - b
				d = 1 - (fN + b)
