...
sys.path.insert(0, '/home/jmurga/mkt/201902/scripts/src/deprecated')
...
if __name__ == "__main__":
	...
	for index,row in cds[(cds['id']=='FBgn0000028')].iterrows():
		print(index,row['id'])
		start = time.time()
		...
		if((len(refSeq)/3).is_integer()): 
			...
			dmelFourFoldDafDiv['pop'] = args.population
			dmelZeroFoldDafDiv['pop'] = args.population
			
			df = pd.concat([dmelFourFoldDafDiv,dmelZeroFoldDafDiv])
			# df.columns = ['id','chr','POS','div','rawDerivedAllele','type','pop']

			div = df.groupby(['id','type','pop'])['div'].sum().reset_index()
			div = div.pivot_table(index=['id','pop'],columns=['type'],values='div').reset_index()
			div.columns = ['id','pop','di','d0']
			print(div)

			daf = df[['id','rawDerivedAllele','type','pop']][df['rawDerivedAllele']!=0]
			...
			print(sfs[['id','pop','p0','pi']])
			print('++++++++++++++++++++++++++++++++++++++++')
			# Save results to df
			dmelFourFoldDafDiv.to_csv(args.path + '/dmel/alleleFrequencies/' + args.population + 'FourFold.tab',sep='\t',header=False,mode='a',index=False)
			dmelZeroFoldDafDiv.to_csv(args.path + '/dmel/alleleFrequencies/' + args.population + 'ZeroFold.tab',sep='\t',header=False,mode='a',index=False)
